...
// Jobs are pre-generated by scripts/sync-jobs.mjs via GitHub Actions.
// At build time Astro/Vite imports this JSON statically — zero runtime requests.
...
export async function fetchJobs(): Promise<ParsedJob[]> {
  ...
  // Fallback to mock data in local dev when sync hasn't been run yet.
  if (!import.meta.env.PROD) return MOCK_JOBS;
